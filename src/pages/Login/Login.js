...
import { getConfig, getUserInfo } from "../../api/auth";
import UserPool from "../../UserPool";
import { setAuth, setUser, setUserInfo } from "../../features/user/userSlice";
import { setReference } from "../../features/reference/referenceSlice";
...
const Login = () => {
  ...
  const preRequisiteCall = async () => {
    try {
      // Set User detail Data
      const response = await getUserInfo();

      localStorage.setItem('userInfo', JSON.stringify(response.data.response.userInfo));
      dispatch(setUserInfo(response.data.response.userInfo));

      // Set refernce data
      const res = await getConfig();
      localStorage.setItem('reference', JSON.stringify(res.data.data));
      dispatch(setReference(res.data.data));
      dispatch(setAuth())
    } catch (err) {
      console.log(err, 'Error in PreRequisites Calls')
    }
  }
  ...
  const handleLogin = async () => {
    ...
    user.authenticateUser(authDetails, {
      onSuccess: data => {
        const userData = data.getIdToken().payload;
